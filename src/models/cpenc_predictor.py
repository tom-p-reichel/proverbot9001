...
class CPENCDataset(Dataset):
    data : List[CPENCSample]
    def __iter__(self):
...
class CPENClassifier(nn.Module):
    ...
    def forward(self, hyp_tokens : torch.LongTensor, goal_tokens : torch.LongTensor, prev_tactic : torch.LongTensor) \
        -> torch.FloatTensor:
        ...
        full_data = self.squish(torch.cat((hyp_output, self.squish(torch.cat((goal_output, prev_data), dim=1))), dim=1))
    ...
    def run(self,
        ...
        return self(maybe_cuda(hyp_tokens), maybe_cuda(goal_tokens), maybe_cuda(prev_tactic))

class CPENCPredictor(NeuralClassifier[CPENCDataset, 'CPENClassifier']):
    ...
    def _predictDistribution(self, in_data : TacticContext) \
        ...
        return self._model.run(hyp_tensor, goal_tensor, prev_tensor)
    ...
    def predictKTacticsWithLoss_batch(self,
        ...
        with self._lock:
            ...
            prediction_distributions = self._model.run(hyp_tensor, goals_tensor, prevs_tensor)
    ...
    def _encode_tokenized_data(self, data : TokenizedDataset, arg_values : Namespace,
        ...
        -> CPENCDataset:
        return CPENCDataset([CPENCSample(embedding.encode_token(
                get_stem(prev_tactics[-1]) if len(prev_tactics) > 1 else "Proof"),
                                     context, goal, tactic)
                           for prev_tactics, context, goal, tactic in data])
    def _data_tensors(self, encoded_data : CPENCDataset, arg_values : Namespace) \
        -> List[torch.Tensor]:
        prevs, hyps, goals, nexts = zip(*encoded_data)
        ...
        return [hyp_stream, goal_stream, prev_stream, out_stream]

    def _get_model(self, arg_values : Namespace, tactic_vocab_size : int,
                   term_vocab_size : int) \
                   -> CPENClassifier:
        return CPENClassifier(term_vocab_size, arg_values.hidden_size, tactic_vocab_size,
    ...
    def _getBatchPredictionLoss(self, data_batch : Sequence[torch.Tensor],
                                model : CPENClassifier) \
                                -> torch.FloatTensor:
        hyp_batch, goal_batch, prev_batch, output_batch = \
            cast(Tuple[torch.LongTensor, torch.LongTensor, torch.LongTensor, torch.LongTensor], data_batch)
        predictionDistribution = model.run(hyp_batch, goal_batch, prev_batch)
        output_var = maybe_cuda(Variable(output_batch))
        return self._criterion(predictionDistribution, output_var)

    def _description(self) -> str:
       return "cpenc -- pytorch model for proverbot"

def main(arg_list : List[str]) -> None:
    predictor = CPENCPredictor()
